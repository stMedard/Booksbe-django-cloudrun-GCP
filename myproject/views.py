...
from myproject.models import Book, Chapter, Book, Illustration
...
from myproject.forms import NewUserForm, BookForm, ChapterForm, IllustrationForm
...
def edit_book(request, book_id):
    ...
    try:
        ...
        messages.add_message(request, messages.WARNING, 'The book does not exist!')
        return redirect("index")
    
    if book.user_b != request.user:
        return redirect("index")
...
class BookDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        get_object_or_404(Book, pk=book_id, user_b=self.request.user)
        return context
...
class IndexBookDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        book_id = self.kwargs['pk']
        context['chapter_list'] = Chapter.objects.filter(book = book_id)
        get_object_or_404(Book, pk=book_id, publish=True)
        return context
...
class UserBookListView(ListView):
    ...
    def get_queryset(self):
        ...
        return book.filter(user_b=self.request.user) 
...
class PublishedBookListView(ListView):
    ...
    def get_queryset(self, *args, **kwargs):
        book = Book.objects.order_by("-id")
        return book.filter(publish=True) 

class IllustrationListView(ListView):

    model = Illustration
    template_name = 'book/illustration_gallery.html'
    ...
    if not request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if formset.is_valid():    
                ...
                return redirect('illustration-list')
        else:
            formset = IllustrationForm(user=request.user)
...
def illustration_delete(request, illustration_id):

    if not request.user.is_authenticated:
        return render(request, 'accounts/login.html')

    try:
        i = Illustration.objects.get(id=illustration_id)
    except:
        messages.add_message(request, messages.WARNING, 'The illustration does not exist!')
        return redirect("index")
    
    if i.user != request.user:
        return redirect("index")    

    bucket_name = 'booksbe-test-3-media'
    blob_name = str(i.image)
    ...
    try:
        ...
        try:
            ...
            messages.add_message(request, messages.SUCCESS, 'Illustration removed.')
    except:
        messages.add_message(request, messages.WARNING, 'This illustration is book bound, please delete the illustration in the book first!')
...
def edit_chapter(request, chapter_id):
    ...
    try:
        ...
        messages.add_message(request, messages.WARNING, 'The chapter does not exist!')
        return redirect("index")
    
    if chapter.user_c != request.user:
        return redirect("index")
...
def generatePDF(request, id):
    ...
    if book.user_b != request.user and book.publish == False:
        return redirect("index")
    ...
    def go():
        ...
        def doHeading(text,sty):
            ...
            Story.append(h) 
        chapterNum = 0
        for i in sorted(chapters, key=lambda x: getattr(x, 'id')):       
            Story.append(NextPageTemplate('ContentPage'))    
    ...
    try:
        ...
        messages.add_message(request, messages.WARNING, 'To generate a book in PDF, the book must have a chapter!')
        return redirect("/add_chapter")

    if book.illustration is None:
        messages.add_message(request, messages.WARNING, 'To generate a book in PDF, the book must have an illustration!')
